...
if ($generate == "Clear") {
	$result = @mysqli_query($db, "SELECT * FROM orders WHERE date IS NULL ORDER BY orderid DESC LIMIT 10");
	$keepfrom = @mysqli_result($result, 9, "orderid");
	@mysqli_query($db, "DELETE FROM orders WHERE date IS NULL AND orderid<'$keepfrom'");
...
for ($i = 0; $i < @mysqli_num_rows($result); $i++) {
	...
	$timedate = explode(" ", @mysqli_result($result, $i, "tempdate"));
	$tempdate = $timedate[0];
	...
	$thiscustomerid = @mysqli_result($result, $i, "customerid");
	if ($thiscustomerid == "0") $customername = "Unknown";
	else {
		$sql = "SELECT * FROM shipping WHERE shippingid='$thiscustomerid'";
		$result3 = @mysqli_query($db, "$sql");
		$shippingfirstname = @mysqli_result($result3, 0, "shippingfirstname");
		$shippinglastname = @mysqli_result($result3, 0, "shippinglastname");
	...
	echo "</font></td><td><font face=\"Arial, Helvetica, sans-serif\" size=\"1\">$orderid</font></td><td><font face=\"Arial, Helvetica, sans-serif\" size=\"1\">$displaydescr</font></td><td><font face=\"Arial, Helvetica, sans-serif\" size=\"1\">".$currencysymbols[$ashopcurrency]["pre"].number_format($price,$showdecimals,$decimalchar,$thousandchar).$currencysymbols[$ashopcurrency]["post"]."</font></td><td><font face=\"Arial, Helvetica, sans-serif\" size=\"1\">$customername</font></td>";
