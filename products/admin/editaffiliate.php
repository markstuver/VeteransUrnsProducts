...
$result = @mysqli_query($db, "$sql");
$user = @mysqli_result($result, 0, "user");
...
$signedup = @mysqli_result($result, 0, "signedup");
$lastupdated = @mysqli_result($result, 0, "updated");
...
if (!empty($referredby)) {
	$result2 = @mysqli_query($db, "SELECT * FROM affiliate WHERE affiliateid='$referredby'");
	$sponsorfirstname = @mysqli_result($result2, 0, "firstname");
	$sponsorlastname = @mysqli_result($result2, 0, "lastname");
}

// Check if this affiliate exists as a Sales Office user too...
$salesrep = FALSE;
if (file_exists("$ashoppath/emerchant/quote.php")) {
	$result3 = @mysqli_query($db, "SELECT * FROM emerchant_user WHERE username='$user'");
	if (@mysqli_num_rows($result3)) $salesrep = TRUE;
}
...
if ($makesalesrep && $affiliateid) {
	...
	$result3 = @mysqli_query($db, "SELECT * FROM emerchant_user WHERE username='$user'");
	if (@mysqli_num_rows($result3)) $msg = "<p align=\"center\" class=\"notconfirm\">".THEREISALREADYAUSER."</p>";
	else {
		@mysqli_query($db, "INSERT INTO emerchant_user (username,password) VALUES ('$user','$passhash')");
