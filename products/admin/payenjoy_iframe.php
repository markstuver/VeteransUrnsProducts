...
if ( ! function_exists('signInfo') ) {
	function signInfo ($merchantnumber, $gatewaynumber, $orderid, $amount, $secret, $currency)
	{
		...
		$signinfo = hash('sha256', $merchantnumber.$gatewaynumber.$orderid.$currency.$amount."$ashopurl/admin/payenjoy.php".$secret);
		return $signinfo;
	}
}
...
if ($testmode) $payenjoyurl = "https://payment.seeipay.com/TestInterface";
else $payenjoyurl = "https://payment.seeipay.com/Interface";
...
if ($merNo == $merchantid && $gatewayNo == $transactionkey && isset($orderNo) && is_numeric($orderNo) && isset($tradeNo) && $orderStatus == 1) {
	...
	echo "<iframe src=\"$ashopurl/admin/payenjoy.php?pendingorder=$invoice&products=$products\" style=\"seamless:seamless; width: 600px; height: 500px; border: none;\"></iframe>";
	if ($lang != $defaultlanguage && file_exists("$ashoppath$templatepath/payment-$lang.html")) ashop_showtemplatefooter("$ashoppath$templatepath/payment-$lang.html");
	else ashop_showtemplatefooter("$ashoppath$templatepath/payment.html");
	...
	<input type=\"hidden\" name=\"returnUrl\" value=\"$ashopurl/admin/payenjoy.php\">
	</form>
	</body>
	</html>";
} else {
	...
	echo "<br /><br /><p align=\"center\"><span class=\"ashopmessageheader\">".ERROR."</span><br /><br /><span class=\"ashopmessage\">$orderInfo<br /><br /><a href=\"$ashopurl/checkout.php\">".TRYAGAIN."</a>!</span></p>";
	if ($lang != $defaultlanguage && file_exists("$ashoppath$templatepath/payment-$lang.html")) ashop_showtemplatefooter("$ashoppath$templatepath/payment-$lang.html");
	else ashop_showtemplatefooter("$ashoppath$templatepath/payment.html");
	exit;
}
