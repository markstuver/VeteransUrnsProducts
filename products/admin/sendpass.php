...
if (!empty($r) && !empty($u) && is_numeric($u)) {
	...
	$result = @mysqli_query($db, "SELECT passwordreset FROM user WHERE userid='$u'");
	$resethash = @mysqli_result($result,0,"passwordreset");
	...
	if (empty($newpassword)) {
		...
		if ($newpassword != $confirm) {
			...
			@mysqli_query($db, "UPDATE user SET password='$passwordhash', passwordreset='' WHERE userid='$u'");
			echo "<HTML><HEAD>".CHARSET."<title>$ashopname - ".ADMINPANELLOGIN."</title><meta http-equiv=\"Content-Type\" content=\"text/html; charset=iso-8859-1\"><link rel=\"stylesheet\" href=\"admin.css\" type=\"text/css\"></HEAD>
			<BODY bgcolor=\"#FFFFFF\" text=\"#FFFFFF\">
			<CENTER>
			<table width=\"100%\" height=\"100%\"><tr><td align=\"center\">
...
if ($user != "") {
	...
	if (@mysqli_num_rows($result) != 0) {
		...
		$uid = @mysqli_result($result, 0, "userid");
  ...
  if ($user && $email && $uid) {
	  ...
	  @mysqli_query($db, "UPDATE user SET passwordreset='$resethash' WHERE username='$user'");
